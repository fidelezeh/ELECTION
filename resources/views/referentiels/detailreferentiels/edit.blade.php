        ...
        <div class="p-2 bg-white rounded-tl-lg rounded-tr-lg">
            <div class="flex items-center justify-between">
                <h2 class="p-1 text-lg md:text-lg text-black font-normalleading-tight flex items-center justify-start">
                    MODIFICATION DU DETAIL REFERENTIEL ({{ $laligne?->libelle }})
                </h2>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white -mt-3 overflow-hidden border border-gray-200 shadow-sm sm:rounded-lg">
        <section class="container w-full p-6 pb-0 mx-auto shadow-md shadow-black rounded-bl-lg rounded-br-lg">
            <div class="-mt-6 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{route('detailreferentiel.update', $laligne?->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4">
                        <input type="hidden" name="referentiel_id" value="{{ $laligne?->referentiel_id }}">
                        <fieldset>
                            <div class="pt-4">
                            ...
                            <div class="pt-4">
                                ...
                                <div class="relative mt-1">
                                    <select name = "parent_id" class="mt-2 block w-full rounded-md border-gray-300">
                                        @forelse  ($laligne?->leparent as $v)
                                            <option value=" {{ $v->id }} " {{ ($v->id == $laligne->parent_id)?"selected":"" }}> {{ $v->libelle }}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                    @error('parent_id')
                    ...
                    <div class="grid md:flex grid-cols-2 justify-start gap-4 mt-3 p-2">
                        <a href="{{route('sousRef.index', $laligne?->referentiel_id)}}">
